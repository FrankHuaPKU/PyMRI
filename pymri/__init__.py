...
class ScalarField:
    ...
    def __array_ufunc__(self, ufunc, method, *inputs, **kwargs):
        ...
        if isinstance(results, tuple):
            # 多返回值情况
            return tuple(
                ScalarField(result, [self.Lx, self.Ly, self.Lz]) 
                if isinstance(result, np.ndarray) else result 
                for result in results
            )
        elif isinstance(results, np.ndarray):
            # 返回数组, 转换为ScalarField
            return ScalarField(results, [self.Lx, self.Ly, self.Lz])
        else:
            # 返回标量值
            return results
    ...
    def __add__(self, other):
        ...
        if not isinstance(other, ScalarField):
            raise TypeError("ScalarField只能与另一个ScalarField相加")
            
        if (self.box != other.box):
            raise ValueError("两个ScalarField的box尺寸必须相同")
            
        if self.data.shape != other.data.shape:
            raise ValueError("两个ScalarField的数组形状必须相同")

        return ScalarField(self.data + other.data, [self.Lx, self.Ly, self.Lz])
    ...
    def __neg__(self):
        ...
        """
        return ScalarField(-self.data, [self.Lx, self.Ly, self.Lz])
    ...
    def __sub__(self, other):
        ...
        """
        if not isinstance(other, ScalarField):
            raise TypeError("ScalarField只能与另一个ScalarField相减")
    ...
    def __mul__(self, other):
        ...
        if isinstance(other, (float, int)):
            return ScalarField(self.data * other, [self.Lx, self.Ly, self.Lz])
    ...
    def __rmul__(self, other):
        ...
        """
        if isinstance(other, (float, int)):
            return ScalarField(other * self.data, [self.Lx, self.Ly, self.Lz])
        elif isinstance(other, VectorField):
            # 委托给VectorField处理
